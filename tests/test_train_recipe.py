...
def _normalize_rendered_target_header(rendered: str) -> str:
    return rendered.replace(
        "Generated by TEVO TrainRecipe bridge for autoresearch_at_home_cuda",
...
def test_apply_train_recipe_to_autoresearch_at_home_matches_upstream_cuda() -> None:
    recipe = train_recipe_from_spec(_compatible_spec(), candidate_id="at-home-cand")
    upstream = apply_train_recipe_to_source(
        CUDA_SAMPLE,
        recipe,
        TrainRecipeTarget.AUTORESEARCH_CUDA,
    )
    at_home = apply_train_recipe_to_source(
        CUDA_SAMPLE,
        recipe,
        TrainRecipeTarget.AUTORESEARCH_AT_HOME_CUDA,
    )
    assert _normalize_rendered_target_header(at_home) == upstream
    assert (
        train_recipe_projection_applied(recipe, TrainRecipeTarget.AUTORESEARCH_AT_HOME_CUDA)
        is False
    )
...
def test_cli_train_recipe_export_and_render(tmp_path: Path) -> None:
    ...
    at_home_train_py = tmp_path / "at_home_train.py"
    at_home_train_py.write_text(CUDA_SAMPLE)
    at_home_render = runner.invoke(
        app,
        [
            "train-recipe-render",
            str(recipe_path),
            "--backend",
            "autoresearch_at_home_cuda",
            "--train-py",
            str(at_home_train_py),
        ],
    )
    assert at_home_render.exit_code == 0, at_home_render.output
    assert _normalize_rendered_target_header(at_home_train_py.read_text()) == train_py.read_text()
