...
# === TEVO TRAIN RECIPE: NORM START ===
def norm(x):
    if NORM_KIND == "layernorm":
        return F.layer_norm(x, (x.size(-1),))
    return F.rms_norm(x, (x.size(-1),))
# === TEVO TRAIN RECIPE: NORM END ===
# === TEVO TRAIN RECIPE: VALUE_EMBED START ===
...
# === TEVO TRAIN RECIPE: VALUE_EMBED END ===
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        cos, sin = cos_sin
# === TEVO TRAIN RECIPE: QK_NORM START ===
        q, k = apply_rotary_emb(q, cos, sin), apply_rotary_emb(k, cos, sin)
        if USE_QK_NORM:
            q, k = norm(q), norm(k)
# === TEVO TRAIN RECIPE: QK_NORM END ===
...
# === TEVO TRAIN RECIPE: MLP START ===
def apply_mlp_activation(x):
...
# Generated by TEVO TrainRecipe bridge for autoresearch_cuda
# Recipe: tune_attn_sparsity-5-ecda
# Source candidate: tune_attn_sparsity-5-ecda
# Model architecture
DEPTH = 8
MODEL_DIM = 512
N_HEAD = 4
...
# === TEVO TRAIN RECIPE: MODEL_CONFIG START ===
...
# === TEVO TRAIN RECIPE: MODEL_CONFIG END ===
