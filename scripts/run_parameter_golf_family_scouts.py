"""Plan and smoke-test seeded Parameter Golf family scout searches."""
...
from transformer_evolution_llm.parameter_golf_family_scout import (  # noqa: E402
    build_scout_plan,
    run_local_smoke_scouts,
)

app = typer.Typer(help="Seeded Parameter Golf family scout helpers.", no_args_is_help=True)
console = Console()
...
def plan_cmd(
    ...
    """Show the recommended multi-family search order and commands."""
    plan = build_scout_plan(stage=stage, device=device, seed=seed, family_ids=family)
    if as_json:
        console.print_json(data=plan)
        return

    table = Table(title=f"Parameter Golf {stage.title()} Family Plan")
    ...
    for row in plan:
        ...
        table.add_row(
            ...
            f"{budget['generations']}g/{budget['steps']}s/{budget['eval_batches']}e",
            str(row["note"]),
        )
    console.print(table)
    for row in plan:
...
def smoke_cmd(
    out_root: Annotated[Path, typer.Option(help="Directory to write smoke artifacts into.")] = Path(
        "runs/parameter_golf_family_scout_smoke"
    ),
    ...
    """Run a tiny local smoke search for the seeded family lanes."""
    report = run_local_smoke_scouts(
        out_root,
        family_ids=family,
...
def write_plan_cmd(
    ...
    """Persist the recommended family plan as JSON."""
    plan = build_scout_plan(stage=stage, device=device, seed=seed, family_ids=family)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(plan, indent=2), encoding="utf-8")
    console.print(f"[bold green]Wrote family plan:[/] {out_path}")
